...
gen.data.C <- function(N, seed){
  ...
  for (i in 1:1000) {
    ...
    for (k in 1:5) {
      ...
      # True RF (full predictors)
      model_rf_0 <- ranger(y ~ x1 + x12 + x2 + x22 + x_12 + e1 + e2,
                           data = train_data_0, num.trees = 300, mtry = 2, min.node.size = 5)
      murf0[valid_idx] <- predict(model_rf_0,
                                  data = valid_data[, c("x1","x12","x2","x22","x_12","e1","e2")])$predictions
      
      model_rf_1 <- ranger(y ~ x1 + x12 + x2 + x22 + x_12 + e1 + e2,
                           data = train_data_1, num.trees = 300, mtry = 2, min.node.size = 5)
      murf1[valid_idx] <- predict(model_rf_1,
                                  data = valid_data[, c("x1","x12","x2","x22","x_12","e1","e2")])$predictions
