...
gen.data.A <- function(N, seed){
  ...
  for (i in 1:1000) 
  {
    ...
    y0 <- 20 - 5 * x1 + 2 * x12 - 5 * x2 + 2 * x22 + 4 * x_12 + e1*tr + e2*tr + eps0
    ...
    mod.psf1 <- glm(tr ~ x1 + x2 + x22 + x_12 + e1+ e2 , family = binomial(link = "logit"), data = datat)
    ...
    mod0f <- lm(y ~ x1 + x2 + x22 + x_12 + e1 + e2, data = datat0)
    ...
    for (k in 1:5) {
      ...
      # Train random forest on treated (Tr == 1)
      mod_rf_1 <- ranger(y ~ x1  + x2 + x22 + x_12 + e1 + e2, data = data1_train, num.trees = 300, mtry = 2, min.node.size = 5)
      murf1_f[valid_idx] <- predict(mod_rf_1, data = valid_data[, c("x1", "x2", "x22", "x_12", "e1", "e2")])$predictions
